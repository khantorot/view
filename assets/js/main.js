...
function showPage() {
    ...
    for (key in projects) {
        if (projects[key].name.replace(/\s/g, '') == main.getAttribute('data-name').replace(/\s/g, '')) {
            let last = Number(Object.keys(projects).length - 1);
            let next = (+key == last) ? 0 : +key + 1;

            if (preloader.getAttribute('data-content') == 'video' || window.innerWidth < 900) {
                preloader.style.backgroundImage = 'url("../../content/images/' + projects[key].content + '.gif")';
                preloader.innerHTML = "";
            }
            ...
            about_out += '<h4>director&#39s notes</h4>';
            ...
            footer_out += '<a href="../' + projects[next].name.replace(/\s/g, '') + '/' + projects[next].name.replace(/\s/g, '') + '.html" class="next_project">'
            footer_out += '<h2>' + projects[next].name + '<span> .' + get_num(next) + '</span></h2>'
            footer_out += '<div style="background-image: url(../../content/images/' + projects[next].content + '.gif)"></div></a>'
